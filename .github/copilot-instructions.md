...
- `src/forgejo.rs` - Forgejo API interaction and cloning logic
...
- Forgejo API v1 integration for self-hosted instances
...
- Symbolic link creation for git submodules (all 6 git servers)
- Bearer token and basic auth support
- Token-based clone URL injection (Azure DevOps, Bitbucket, Forgejo, Gitea, GitHub, GitLab)
...
- Forgejo: `FORGEJO_TOKEN` environment variable or CLI token
...
- **integration.yml**: Runs integration tests against all 6 git servers
  - Mock Azure DevOps (Azure DevOps clone with superpull)
  - Bitbucket Cloud (Bitbucket clone with superpull)
  - Forgejo (Forgejo clone with superpull)
...
- Support for additional self-hosted Git services (Gogs, etc.)
