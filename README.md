...
- **FojClone**: Clone all repositories from a self-hosted Forgejo instance
...
¹ Forgejo and Gitea use positional `<base_url>` argument instead of `-s` flag (e.g. `foj-clone https://forgejo.example.com org-name` or `gea-clone https://gitea.example.com org-name`)
...
```bash
superpull foj-clone [-a token] [-l] [-m size] [-n regex] [-o regex] <base_url> <organization> [folder]
```
...
superpull foj-clone https://forgejo.example.com myorg ./myorg-repos

# Clone repos from Forgejo with filtering
superpull foj-clone -p 5 -n "^api-" https://forgejo.example.com myorg ./api-repos

# Clone repos from Forgejo and create symlinks for submodules
superpull foj-clone -l -n "^v2-" https://forgejo.example.com myorg ./v2-repos
