...
pub async fn super_clone(
    ...
    clone_task_manager::execute_clone_tasks(repo_urls, target_folder, options.clone(), |url| {
            ...
            && let Some(index) = clone_url.find("://")
        {
            ...
            let encoded =
                base64::engine::general_purpose::STANDARD.encode(format!("PAT:{}", azure_token));
            clone_url = format!(
                "{}{}@{}",
                &clone_url[..index + 3],
                encoded,
...
async fn get_repos(
    ...
    all_repos.retain(|r| filter_opts.should_include(&r.name, r.is_fork, r.size));
...
async fn get_projects(
    ...
    let server_url_str = if let Some(url) = server_url {
        url.trim_end_matches('/')
    } else {
        AZURE_SAAS_API
    };
    let api_url = format!("{}/{}/_apis/projects", server_url_str, organization);
    let client = Client::new();
    let auth = base64::engine::general_purpose::STANDARD.encode(format!("PAT:{}", token));
    let mut projects = Vec::new();
    let mut url = format!("{}?api-version=7.1&$top={}", api_url, PER_PAGE);
    ...
    loop {
        ...
        if projects_response.value.is_empty() {
            break;
        }

        projects.extend(projects_response.value);

        if let Some(token_str) = &continuation_token {
            url = format!(
                "{}?api-version=7.1&$top={}&continuationToken={}",
                api_url, PER_PAGE, token_str
            );
        } else {
            break;
        }
...
async fn get_repos_for_project(
    ...
    let api_url = format!("{}/{}/_apis/git/repositories", server_url_str, organization);
    let client = Client::new();
    let auth = base64::engine::general_purpose::STANDARD.encode(format!("PAT:{}", token));
    let mut repos = Vec::new();
    let mut url = format!(
        "{}?project={}&api-version=7.1&$top={}",
        api_url, project, PER_PAGE
    );
    ...
    loop {
        ...
        if repos_response.value.is_empty() {
            break;
        }

        // Add project name to each repository
        repos.extend(repos_response.value.into_iter().map(|mut r| {
        ...
        if let Some(token_str) = &continuation_token {
            url = format!(
                "{}?project={}&api-version=7.1&$top={}&continuationToken={}",
                api_url, project, PER_PAGE, token_str
            );
        } else {
            break;
        }
