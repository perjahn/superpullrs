...
const AZURE_API: &str = "https://dev.azure.com";
...
pub async fn super_clone(
    ...
    let repos = get_all_repos(organization, &azure_token, server_url, filter_opts).await?;
...
async fn get_projects(
    ...
    let projects_response: AzureProjectsResponse = response.json().await?;
...
async fn get_repos_for_project(
    ...
    let repos_response: AzureRepositoriesResponse = response.json().await?;
    Ok(repos_response.value)
}
