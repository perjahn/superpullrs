...
pub async fn super_clone(
    ...
    let repos = get_repos(server_url, organization, &gitea_token, filter_opts).await?;

    if repos.is_empty() {
        if gitea_token.is_none() {
            println!(
                "No git repos found. GITEA_TOKEN environment variable isn't set, for access to private repos it must be set."
            );
        } else {
