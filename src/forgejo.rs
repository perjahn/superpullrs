...
pub async fn super_clone(
    ...
    let forgejo_token = token
        .map(|t| t.to_string())
        .or_else(|| env::var("FORGEJO_TOKEN").ok());
...
async fn get_repos_paginated(
    ...
    loop {
        ...
        let _ = debug_utils::save_api_response("forgejo", "repos", &response_text);
        let page_repos: Vec<ForgejoRepository> = serde_json::from_str(&response_text)?;
